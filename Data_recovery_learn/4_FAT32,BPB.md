...
5. DATA数据区中的隐藏文件夹 **$RECYCLEBIN** 对应的子目录中，新增**2个文件**的目录项（0x24号簇即起始扇区为8592的文件，记录了被删除文件的原位置；0x23号簇即起始扇区为8584的文件，记录了被删除文件的内容，其实，该文件的目录项中的文件起始簇号记录，同被删除文件的目录项中的文件起始簇号记录，文件的内容还在原地，没有被移动，也没有被修改）
...
4. 隐藏文件夹 **$RECYCLEBIN** 对应的子目录中，相应2个文件各自的目录项的第1个字节被改为 **0xE5**
